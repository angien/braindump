...
if (!$link) { 
  ...
  // pull all braindumps
$result = mysqli_query($link, "SELECT * FROM courses, braindumpers, users, webex WHERE courses.course_id = braindumpers.course_id AND
  braindumpers.id = users.id AND
  courses.course_id = webex.course_id");

echo "<table class='table'>";
if($result && (mysqli_affected_rows($link)>0)) {
  echo "<tr class='label-primary'><th>Title</th><th>Description</th><th>Date</th><th>Time</th><th>Webex Link</th><th>Subscribe?</th></tr>";
  while($row = mysqli_fetch_array($result)){
   echo "<tr><td><a href='more.php?course_id=".$row['course_id']."'>".$row['title']."</a></td><td>".$row['description']."</td>";
    ...
    if ($row2['reg_link'] == NULL) {
      echo "<td>There is currently no WebEx meeting.</td>";
    }
    else {
      echo '<td><a href="'.$row2['reg_link'].'">Click</a></td>';
    }

    echo "<td><form action='subscribe.php?course_id=".$row['course_id']."&unsub=0";
    echo "' method='post'>";
    echo "<input type='submit' class='btn btn-default' value='Subscribe'/></form></td></tr>";
  }
}
else {
  echo "<tr>There are no BrainDumps currently.</tr>";
}
echo "</table><br>";
